...
## 2026-09-14 特定RID独占UI与SORT二次确认门限调整
- `UI_TRACK_CONFIRM_HITS` 默认值从 `9` 进一步降为 `7`；未单独配置时，`STRIKE_TRACK_CONFIRM_HITS` 继续跟随该值，因此默认同为 `7`。特定RID仍只消费已通过该二次确认的SORT。
- 新增默认开启的 `SPECIAL_RID_UI_EXCLUSIVE`。当特定RID身份功能启用时，普通SORT仍正常参与内部跟踪、测距、云台控制和打击端候选，但普通UI发送路径不再发送任何 `0x02`；UI目标状态只由特定RID线程发送，目标ID仅为动态分配的1或2。即使特定RID运行时初始化失败，也不会回退并泄漏普通UI目标ID。
- 通用RID测距链路和打击端发送保持启用，打击端目标ID仍沿用原有SORT/打击端身份，不要求限制为1或2；GPS `0x03` 发送不受影响。
- 本次不改变内部 `TRACK_CONFIRM_HITS=3`、SORT 4秒视觉新鲜度、RID 7秒接收新鲜度、轨迹生命周期、RID/SORT匹配规则、UI 34字节协议、打击端21字节协议或云台安全门控。降低门限会缩短初次绑定与重绑定等待时间，同时增加短时稳定虚警进入确认候选和打击候选的风险。
