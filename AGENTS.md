...
## 2026-09-15 SORT重关联门限微调
- 使用 `20260912_032537` 的2871个融合周期完整回放，`TRACK_ASSOCIATION_MAX_DEG=6.0°`、`TRACK_REACQUIRE_MAX_DEG=4.0°` 的基线精确复现271次 `NEW_TRACK`。
- 默认 `TRACK_REACQUIRE_MAX_DEG` 从 `4.0°` 调整为 `5.0°`；同一日志回放的 `NEW_TRACK` 降为260次，达到16次确认的轨迹仍为37条，新增接受的重关联误差最大为 `4.989°`。
- `TRACK_ASSOCIATION_MAX_DEG=6.0°` 保持不变；提高到 `6.5°/7.0°` 在同一回放中只额外减少1/2次建轨，收益不足以抵消多目标错配风险。
- 门控仍采用 `cost >= gate` 阻断，因此恰好 `5.0°` 及更大偏差仍拒绝；动态协方差门限低于 `5.0°` 时，以更低值为准。其他追踪、UI、控制、RID和打击逻辑均未改变。

## 2026-09-14 特定RID独占UI/打击输出与SORT二次确认门限调整
- `UI_TRACK_CONFIRM_HITS` 默认值从 `9` 进一步降为 `7`；未单独配置时，`STRIKE_TRACK_CONFIRM_HITS` 继续跟随该值，因此默认同为 `7`。特定RID仍只消费已通过该二次确认的SORT。
- 新增默认开启的 `SPECIAL_RID_UI_EXCLUSIVE`。当特定RID身份功能启用时，普通SORT仍正常参与内部跟踪、测距和云台控制，但普通UI发送路径不再发送任何 `0x02`；UI目标状态只由特定RID线程发送，目标ID仅为动态分配的1或2。即使特定RID运行时初始化失败，也不会回退并泄漏普通UI目标ID。
- 通用RID测距runtime继续运行，但打击候选现在严格限制为已被特定RID注册表按精确SORT generation归属的轨迹；目标ID直接取对应特定RID槽位的1或2。未归属、SORT ID复用但generation不同、注册表不可用或槽位ID不在1/2时均不发布打击快照。发布前再次核对归属，普通RID/SORT不再向打击端发送。
- 特定RID的SORT视觉新鲜度从4秒延长为6秒；6秒边界仍有效，超过6秒停止特定RID UI发送并进入重新绑定判断。RID接收新鲜度保持7秒。
- 本次不改变内部 `TRACK_CONFIRM_HITS=3`、12秒轨迹生命周期、RID/SORT匹配规则、UI 34字节协议、打击端21字节协议、站点GPS字段或云台安全门控。降低确认门限会增加短时稳定虚警进入候选的风险；延长SORT新鲜度会把旧摄像头来源最多多保留2秒，并相应推迟强制重绑定。
...
- 经纬度编码严格为 `int(coordinate * 100) * 360000`，先向0截断到两位小数对应整数，再乘360000；结果只保留低32位并按二进制补码写入网络字节序有符号 `int32`。例如 `103.456123` 的缩放整数为 `3724200000`，打包字段值为 `-570767296`，底层4字节保持一致。经度在前、纬度在后，两字段均计入原异或校验。
...
## 2026-09-13 两个RID_laser固定UI身份与SORT轨迹族
- 仅源码中以SHA-256摘要配置的两个 `RID_laser` 目标启用 `core/special_rid_identity.py`。以“RID存在且首次绑定一个现有 `ui_confirmed=True` SORT”为注册时刻，先成功者动态获得UI_ID 1，后成功者获得2；UI_ID、SORT generation所有权和轨迹族保持到进程退出。源码、测试和文档不得保存两个真实RID序列的明文。
- 特殊RID的UI `0x02` 仍为34字节 `!BB8sIffffI`，`replaced_target_id=0`。目标ID来自特殊RID槽位；方位、俯仰、距离和威胁来自RID实时预测；SORT只提供视觉有效性、`board` 和 `camera_id`。
- 特殊RID独立5Hz发送：RID最后真实测量后0~5秒实时外推，5~7秒冻结5秒位置，RID接收年龄超过7秒停止；current SORT检测年龄超过6秒也停止。预测点不写回真实测量历史。
- SORT接续复用既有 `ui_confirmed`，不新增第三层计数。同摄像头6°、相邻摄像头9°、12秒内双轨收敛1.5°；候选generation必须晚于RID出现和至少一个族内前代，禁止吸收RID出现前的旧SORT。current SORT在6秒内新鲜时保持粘性；超过6秒后不再等待旧45秒门限，也不再参考可能误绑的旧SORT几何，而是在同一周期从新鲜、确认、未归属SORT中按当前RID方位/俯仰二维角度执行全局一对一最近重绑定。附加延迟由 `SPECIAL_RID_REACQUIRE_DELAY_SECONDS` 配置，默认 `0s`。
- 已归属RID_laser的SORT generation永久从普通UI发送路径排除；RID_laser ID固定占用1/2，普通UI分配器在专用注册表启用时从3开始，避免全局目标ID冲突。普通测距runtime和云台主目标控制保持不变；打击候选与发送现在只允许RID_laser已归属generation和ID 1/2。`logs/20260912_032537` 从 `1789156230.355754` 回放得到：RID_laser目标1/UI1族 `{177}`，目标2/UI2族 `{189,196,202,205,238}`，最终SORT238仍归目标2且未被目标1抢占。
