...
## 2026-09-14 UI/SORT二次确认门限调整
- `UI_TRACK_CONFIRM_HITS` 默认值从 `16` 降为 `9`；未单独配置时，`STRIKE_TRACK_CONFIRM_HITS` 继续跟随该值，因此默认同为 `9`。
- 特定RID只消费已通过该二次确认的SORT。按 `logs/20260914_000001` 回放统计，后段SORT 104和108的最大连续命中均为9，调整后可进入特定RID重绑定候选集合。
- 本次不改变内部 `TRACK_CONFIRM_HITS=3`、SORT 4秒视觉新鲜度、RID 7秒接收新鲜度、轨迹生命周期或RID/SORT匹配规则。降低门限会缩短重绑定等待时间，同时增加短时稳定虚警进入UI、打击与特定RID候选集合的风险。
