...
## 2026-09-13 打击端站点GPS坐标协议
- 打击端目标包现为21字节：主体 `!2sBBHHhii` 后接1字节主体异或校验和 `55AA` 帧尾。长度字段为 `0x15`；新增字段放在俯仰之后，顺序为经度 `int32`、纬度 `int32`。
- 坐标编码采用 `int(coordinate * 100) * 3600` 的向0截断语义。实现使用十进制转换规避二进制浮点在两位小数边界处误截断。
...
- UI只接收具有正数有限最终距离的目标。RID绑定目标发生UI ID切换后，新ID的连续3个成功 `0x02` 包携带旧ID；距离无效、没有发送或UDP失败不扣次数，多次快速切换按FIFO通知。
- 状态按RID唯一 `key` 隔离。视觉测距不创建替换通知；RID已替换的旧ID即使重新获得视觉距离也不发送，除非该ID重新绑定任意RID，此时取消待删除和视觉抑制以防误删。
- 旧SORT轨迹短时退出 `ui_tracks` 不会单独产生切换：3秒内恢复仍沿用原UI ID，内部可保留12秒等待同ID重关联。RID暂时无目标或距离过期也不清状态，仅RID连续300秒无上报并永久删除时清理。
- 修改仅位于UI发送协议和发送侧RID身份状态；RID四点匹配、SORT生命周期、双距离裁决、视觉测距、云台线程、打击安全条件和GPS `0x03` 包保持不变。
...
## 2026-07-24 SORT–RID四点方位轨迹匹配
- RID分支已将原先“当前方位最近、无门限强制分配”的UI配对改为四点轨迹门控。
- 候选SORT范围严格等于当前 `ui_tracks/ui_ids`，不使用内部保留但尚未达到UI条件的轨迹。
- 匹配只使用地图方位角，不使用俯仰角；设备朝向和theta标定保持不变。
- 每个 `(SORT track_id, RID key)` 只在 `rid_measurement_seq` 变化时加入一个同步残差点，重复序号不增加窗口。
- SORT方位先按 `rid_render_timestamp` 插值，再计算 `d=wrap(SORT_map_az-RID_map_az)`；最近4点的中位偏差记为 `bij`。
- 综合安装偏差默认 `b0=3.1198935°`；硬门控为 `Ebias=|wrap(bij-b0)|<=2.0°` 和 `Eshape95=P95(|wrap(d-bij)|)<=2.5°`。
- 不足4点时不绑定；超过门限时拒绝；仅通过门控的候选进入匈牙利一对一分配，全部拒绝时不会强制配对。
- 匹配成功后的UI包仍由SORT提供目标ID、地图方位、俯仰和相机来源，RID只提供距离；UI UDP包格式没有变化。
...
- 当前默认 simple 画面关联/回填像素门限为 `3000px`，可通过 `GIMBAL_VISION_SIMPLE_ASSOC_MAX_PX` 调整。该值是低严格度诊断基线，用于优先观察云台画面和测距链路，不应作为正式多目标防错绑门限；它不参与决定一帧是否进入距离 runtime。
